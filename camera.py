    ...
    def get_frame(self):
        ...
        if len(lmList) != 0:
            ...
            if finger[1] == 1 and finger[2] == 1:  # Checks to see if the pointing finger is up and thumb finger is down
                ...
                cX = pX + (x3 - pX) / 1.0001  # Stores previous x locations to update current x location
                cY = pY + (y3 - pY) / 1.0001  # Stores previous y locations to update current y location
